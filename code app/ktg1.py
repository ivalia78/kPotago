...
# --- CSS custom untuk UI (tabel dan button berwarna menarik) ---
...
for key, default in {
    ...
    "username": "",
    "role": "",
    "metode_pembayaran": metode_list[0],
    "ekspedisi": ekspedisi_list[0],
}.items():
...
def add_order_jurnal(order):
    ...
    data["next_id"] += 1
    jurnal_entry = {
        "id": jurnal_id, "tanggal": tgl, "deskripsi": deskripsi,
        "entri": [
...
def tambah_jurnal_baru(data):
    ...
    if st.button("Simpan Jurnal Baru"):
        ...
        jurnal_baru = {
            "id": data["next_id"], "tanggal": tanggal.strftime("%Y-%m-%d"),
            "deskripsi": deskripsi, "entri": entri_terakumulasi
        }
...
def jurnal_umum(data):
    ...
    for jurnal in jurnal_urut:
        ...
        # Buat DataFrame untuk tabel entri jurnal
        df_entri = pd.DataFrame(jurnal["entri"])
        # Format angka Rupiah dan kosongkan angka 0
        def format_rp(x):
...
def app():
    local_css()  # Panggil CSS custom agar styling berjalan
    ...
    if st.session_state.step == 0:
        ...
        st.session_state.jenis_kentang = st.radio(
            "Jenis Kentang",
            ["Kentang Kecil", "Kentang Besar"],
            index=["Kentang Kecil", "Kentang Besar"].index(st.session_state.get('jenis_kentang', "Kentang Besar")),
        ...
        with col1:
            ...
            kg = st.session_state.get('kg', 0)
            jenis_kentang = st.session_state.get('jenis_kentang', '').strip().lower()
            if st.session_state.jenis_kentang == "Kentang Kecil":
                harga_per_kg = 8700
            else:
                harga_per_kg = 10300
            total_kentang = kg * harga_per_kg
            st.session_state.total_kentang = total_kentang
            st.session_state.total_biaya_kentang = total_kentang
            st.write(f"Total belanja anda adalah: Rp {total_kentang:,}")
        ...
        with col1:
            st.session_state.nomor_rekening = st.write("🏦BRI:\n1270 0102 0316 504")
            st.write("Syifaurrahmah")
        ...
        selected_bank = st.session_state.get('metode_pembayaran', 'BRI')
        biaya_admin = biaya_admin_dict.get(selected_bank, 0)
        st.write(f"Anda memilih bank: **{selected_bank}**")
        st.write(f"Biaya Admin: **Rp {biaya_admin:,}**")
        ...
        total_kentang = st.session_state.get('total_kentang', 0)
        st.session_state.harga_keseluruhan = total_kentang + biaya_admin
        st.write(f"💰 **Total yang harus dibayar: Rp {st.session_state.harga_keseluruhan:,}**")
        ...
        st.markdown(f"***Total Keseluruhan: {st.session_state.harga_keseluruhan}***")
